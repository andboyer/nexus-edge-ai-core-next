//! YOLO-World ORT smoke test — loads `models/yolo_world_v2_s.onnx` +
...
//! `NEXUS_TEST_YOLO_WORLD_MODEL` is not set / does not exist. Locally:
//!
//! ```bash
...
//! ```
//!
//! Like `yolo_smoke`, the assertion is intentionally weak — a synthesized
//! frame rarely fires a strong open-vocab detection, so we only require
//! the session ran and any returned detections are structurally valid
//! and labelled with one of the prompts we passed in.
...
fn paths() -> Option<(PathBuf, PathBuf)> {
    let model = std::env::var("NEXUS_TEST_YOLO_WORLD_MODEL")
        .ok()
        .map(PathBuf::from)?;
    let manifest = std::env::var("NEXUS_TEST_YOLO_WORLD_MANIFEST")
        .ok()
        .map(PathBuf::from)?;
    if !model.exists() {
        eprintln!(
            "[yolo_world_smoke] NEXUS_TEST_YOLO_WORLD_MODEL={} not found, skipping",
            model.display()
        );
        return None;
    }
    if !manifest.exists() {
        eprintln!(
            "[yolo_world_smoke] NEXUS_TEST_YOLO_WORLD_MANIFEST={} not found, skipping",
            manifest.display()
        );
        return None;
    }
...
fn synth_frame(w: u32, h: u32) -> Frame {
    // Diagonal gradient — same shape as yolo_smoke, plus a vertical bar
    ...
    Frame {
        ...
        data: Arc::new(data),
        trace_id: "yolo-world-smoke".into(),
    }
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn yolo_world_smoke_runs_on_synthetic_frame() {
    let Some((model, manifest)) = paths() else {
        return; // skip when artifacts not staged locally
    };

    let vocab = load_vocab_from_manifest_public(&manifest, "yolo_world_v2_s")
    ...
    let det = YoloWorldDetector::open(&model, 640, 640, 0.10, 0.50, vocab.clone(), &[])
    ...
    eprintln!(
        "[yolo_world_smoke] {} detections (subset={:?}, vocab_len={})",
        dets.len(),
        prompts,
        vocab.len()
    );
